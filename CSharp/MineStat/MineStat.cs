...
namespace MineStatLib
{
    ...
    public class MineStat
    {
        ...
        static private string strip_motd_formatting(XElement rawmotd)
        {
            ...
            string stripped_motd = rawmotd.Element("text")?.Value;
            if (rawmotd.Elements("extra").Any())
            {
                IEnumerable<XElement> json_data = rawmotd.Element("extra").Elements();
                foreach (XElement item in json_data)
                {
        ...
        public ConnStatus RequestWithRaknetProtocol()
        {
            ...
            byte[] raknetMagic = new byte[] { 0x00, 0xFF, 0xFF, 0x00, 0xFE, 0xFE, 0xFE, 0xFE, 0xFD, 0xFD, 0xFD, 0xFD, 0x12, 0x34, 0x56, 0x78 };

            List<byte> raknetPingHandshakePacket = new List<byte>() { 0x01 };

            byte[] unixtime = BitConverter.GetBytes(DateTimeOffset.Now.ToUnixTimeMilliseconds());
            ...
            int sendlen = sock.Send(raknetPingHandshakePacket.ToArray(), raknetPingHandshakePacket.Count());
            ...
            try
            {
                IPEndPoint endpoint = new System.Net.IPEndPoint(System.Net.IPAddress.Any, Port);
                Queue<byte> response = new Queue<byte>(sock.Receive(ref endpoint));

                if (response.Dequeue() != 0x1c)
                    return ConnStatus.Unknown;

                // responseTimeStamp & responseServerGUID discarded
                long responseTimeStamp = BitConverter.ToInt64(readbytestream(response, 8), 0);
                long responseServerGUID = BitConverter.ToInt64(readbytestream(response, 8), 0);
                ...
                ushort responsePayloadLength = BitConverter.ToUInt16(readbytestream(response, 2), 0);
        ...
        {
            string[] keys = new string[] {"edition", "motd_1", "protocol_version", "version", "current_players", "max_players",
      "server_uid", "motd_2", "gamemode", "gamemode_numeric", "port_ipv4", "port_ipv6"};

            Dictionary<string, string> dic = keys.Zip(payload.Split((char)59), (k, v) => new { k, v })
        ...
        public ConnStatus RequestWithJsonProtocol(NetworkStream stream)
        {
            ...
            // - The packet id 0x00
            List<byte> jsonPingHandshakePacket = new List<byte> { 0x00 };
            ...
            byte[] serverAddr = Encoding.UTF8.GetBytes(Address);
            ...
            byte[] serverPort = BitConverter.GetBytes(Port);
            ...
            int responsePacketId = ReadLeb128Stream(stream);
            ...
            int responsePayloadLength = ReadLeb128Stream(stream);

            // Receive the full payload
            byte[] responsePayload = NetStreamReadExact(stream, responsePayloadLength);
            string responseString = Encoding.UTF8.GetString(responsePayload);
        ...
        private ConnStatus ParseJsonProtocolPayload(byte[] rawPayload)
        {
            try
            {
                XmlDictionaryReader jsonReader = JsonReaderWriterFactory.CreateJsonReader(rawPayload, new System.Xml.XmlDictionaryReaderQuotas());
                ...
                // the MOTD
                XElement descriptionElement = root.XPathSelectElement("//description");
                ...
                XElement playerSampleElement = root.XPathSelectElement("//players/sample");
                if (playerSampleElement != null && playerSampleElement.Attribute(XName.Get("type"))?.Value == "array")
                {
                    IEnumerable<XElement> playerSampleNameElements = root.XPathSelectElements("//players/sample/item/name");
        ...
        public ConnStatus RequestWithExtendedLegacyProtocol(NetworkStream stream)
        {
            ...
            List<byte> extLegacyPingPacket = new List<byte> { 0xFE, 0x01, 0xFA, 0x00, 0x0B };
            ...
            byte[] reqByteLen = BitConverter.GetBytes(Convert.ToInt16(7 + (Address.Length * 2)));
            // Convert to Big-Endian
            if (BitConverter.IsLittleEndian)
            ...
            byte[] addressLen = BitConverter.GetBytes(Convert.ToInt16(Address.Length));
            ...
            IEnumerable<byte> payloadLengthRaw = responsePacketHeader.Skip(1);
            ...
            ushort payloadLength = BitConverter.ToUInt16(payloadLengthRaw.ToArray(), 0);

            // Receive payload
            byte[] payload = NetStreamReadExact(stream, payloadLength * 2);
        ...
        {
            // Send ping packet with id 0xFE, and ping data 0x01
            byte[] legacyPingPacket = new byte[] { 0xFE, 0x01 };
            ...
            ushort payloadLength = BitConverter.ToUInt16(responsePacketHeader, 0);
        ...
        private ConnStatus ParseLegacyProtocol(byte[] rawPayload, SlpProtocol protocol = SlpProtocol.ExtendedLegacy)
        {
            ...
            string payloadString = Encoding.BigEndianUnicode.GetString(rawPayload, 0, rawPayload.Length);

            // This "payload" contains six fields delimited by a NUL character, see below
            string[] payloadArray = payloadString.Split('\0');
        ...
        public ConnStatus RequestWithBetaProtocol(NetworkStream stream)
        {
            ...
            byte[] betaPingPacket = new byte[] { 0xFE };
        ...
        {
            TcpClient tcpclient = new TcpClient { ReceiveTimeout = Timeout * 1000, SendTimeout = Timeout * 1000 };

            Stopwatch stopWatch = new Stopwatch();
            stopWatch.Start();

            // Start async connection
            IAsyncResult result = tcpclient.BeginConnect(Address, Port, null, null);
            // wait "timeout" seconds
            bool isResponsive = result.AsyncWaitHandle.WaitOne(TimeSpan.FromSeconds(Timeout));
        ...
        private static byte[] NetStreamReadExact(NetworkStream stream, int size)
        {
            ...
            do
            {
                byte[] tempBuffer = new byte[size - totalReadBytes];
                int readBytes = stream.Read(tempBuffer, 0, size - totalReadBytes);
